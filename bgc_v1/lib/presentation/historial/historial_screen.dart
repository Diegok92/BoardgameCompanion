...
class _HistorialScreenState extends ConsumerState<HistorialScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      notifier.initialFetch();
      notifier.refreshLocalMatches();
    });
  }
  ...
  void _handleContinueMatch(Partida partida) {
    ...
    if (playerCount == null) {
      CustomAlert.show(
        context,
        'No se pudo determinar la cantidad de jugadores para continuar.',
        isError: true,
      );
      return;
    }
    ...
    String route = '';
    if (partida.id.startsWith('burako_state_')) {
      route = '/burako-tracker';
    } else if (partida.id.startsWith('akropolis_state_')) {
      route = '/akropolis-tracker';
    } else if (partida.id.startsWith('generala_state_')) {
      route = '/generala-tracker';
    } else if (partida.id.startsWith('truco_state_')) {
      route = '/truco-tracker';
    } else if (partida.id.startsWith('scoreboard_state_')) {
      route = '/standard-scoreboard';
    } else if (partida.id.startsWith('tracker_state_')) {
      route = '/hp-tracker';
    } else {
    ...
    context.push(
      route,
      extra: {'playerCount': playerCount, 'fullKey': partida.id},
    );
  }
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text(
            '¿Qué deseas hacer con esta partida que dejaste a medias?',
          ),
          actions: [
            TextButton(
              onPressed: () {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text(
            '¿Estás seguro de borrar este registro del historial? Esta acción no se puede deshacer.',
          ),
          actions: [
            TextButton(
            ...
            FilledButton(
              ...
              onPressed: () {
                ...
                ref
                    .read(historialProvider.notifier)
                    .removeFirestoreMatch(partida.id);
              },
                                ...
                                child: state.isFetchingMore
                                    ...
                                    : TextButton(
                                        onPressed: () => ref
                                            ...
                                            .fetchMore(),
                                        child: const Text(
                                          'Cargar más partidas finalizadas',
                                        ),
                                      ),
                              ),
                            );
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    if (!isEnCurso &&
                                        partida.ganadores.isNotEmpty) ...[
                                      const SizedBox(height: 4),
                                      Text(
                                        partida.isTeamGame
                                            ? 'Ganadores: ${partida.ganadores.join(", ")}'
                                            : 'Ganador: ${partida.ganadores.join(", ")}',
